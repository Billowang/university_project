...
class UltimateJoystickNode(Node):
    def __init__(self):
        ...
        # 設定：加速度限制（每 0.1 秒允許改變的最大比例）
        self.ramp_step = 0.15 

        # Timer 1: 監控連線安全 (Watchdog) - 0.5秒檢查一次
        self.create_timer(0.5, self.watchdog)
        
        # Timer 2: 平滑發送指令 - 0.1秒執行一次
    ...
    def smooth_publish_loop(self):
        ...
        if not is_active:
            ...
            # 處理線性速度 v
            diff_v = target_v - self.current_v
            ...
            # 處理轉向速度 w
            diff_w = target_w - self.current_w
        ...
        msg.linear.z = float(max_rpm)  # 注意：自訂型車輛才會將此欄位挪作他用 (如 RPM)
        self.publisher_.publish(msg)

    def update_targets(self, v, w, web_max, active):
        """ 由 Flask 執行緒呼叫，用來更新目標速度 """
        with self.lock:
...
<head>
    ...
    <title>ROS2 Ultimate Joystick</title>
    <!-- 引入 NippleJS 搖桿函式庫 -->
    <script>
...
<body>
    ...
    <div class="control-panel">
        ...
        <div style="margin-top:15px; font-size: 16px;">
            MAX RPM: <input type="number" id="max_in" value="2000" style="width:100px; padding: 5px; font-size: 16px; text-align: center;">
        </div>
    </div>
    
    <!-- 左右輪即時轉速預估顯示 -->
    <div style="display:flex; justify-content: space-around; padding: 15px; background: #111;">
    ...
    <script>
        ...
        document.addEventListener('gesturestart', function (event) {
            event.preventDefault(); // 阻擋 iOS 放大手勢
        });
        ...
        joy.on('move', (e, d) => {
            if(!active) return;
            curV = d.vector.y;  // 上推為正，下推為負
            curW = -d.vector.x; // 左推為正（逆時針），右推為負（順時針）
            updateUI(curV, curW);
        });
        ...
        function updateUI(v, w) {
            let max = parseFloat(document.getElementById('max_in').value) || 0;
            // 簡易的差速估算邏輯（僅供 UI 預覽參考）
        ...
        // --- 定時心跳包機制 ---
        // 每 100ms 不管搖桿有沒有動，都向後端 ROS 節點報告當前狀態
        setInterval(() => {
            let limit = document.getElementById('max_in').value;
            // 使用 query string 傳遞參數：v(線速度比例), w(角速度比例), max(RPM上限), act(使能開關)
            fetch(`/joy?v=${curV.toFixed(2)}&w=${curW.toFixed(2)}&max=${limit}&act=${active}`)
            .catch((err) => {
                console.log("網路通訊異常: ", err);
            });
        }, 100);
    </script>
</body>
...
def joy():
    ...
    if ros_node: 
        ros_node.update_targets(v, w, m, a)
...
def main():
    ...
    # 啟動 Flask 伺服器 (關閉 debug 模式以防執行緒衝突)
